...
int main(){
   ...
   HashTable testTable(101, hashCode);
   cout << "Testing insertion: 'Daniel' should be inserted at index 77,and 'Joey' should be inserted at index 10" << endl;
   temp = vaccineSerial();
   testTable.insert(Person("Daniel", temp));
   temp = vaccineSerial();
   testTable.insert(Person("Joey", temp));
   testTable.dump();
   cout << endl;


   testTable.insert(Person("Cory", temp));
   testTable.insert(Person("Jason", temp));
   testTable.insert(Person("Nicki", temp));
   testTable.insert(Person("Paula", temp));
   cout << "Testing removal: Removing 'Joey'" << endl;
   testTable.remove(Person("Joey", temp));
   testTable.dump();
   return 0;
}
